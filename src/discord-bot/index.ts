...
import { buildWebullCommands, handleWebullInteraction } from './webull-commands.js';
...
export async function initDiscordBot(): Promise<void> {
  ...
  client.once(Events.ClientReady, async (readyClient) => {
    ...
    try {
      ...
      const commands = [
        ...
        ...buildWebullCommands().map((c) => c.toJSON()),
      ];
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // Route /webull-* commands
    if (interaction.commandName.startsWith('webull-')) {
      try {
        await handleWebullInteraction(interaction);
      } catch (err) {
        console.error('[discord-bot] Webull command error:', err);
        const content = 'Something went wrong processing your command.';
        if (interaction.replied || interaction.deferred) {
          await interaction.followUp({ content, ephemeral: true }).catch(() => {});
        } else {
          await interaction.reply({ content, ephemeral: true }).catch(() => {});
